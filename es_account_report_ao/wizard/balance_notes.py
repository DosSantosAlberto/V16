...
class BanceNotes(models.TransientModel):
    ...
    accounting_year = fields.Many2one( comodel_name='account.fiscal.year',string="Ano Fiscal", default=lambda self: self.env.user.company_id.accounting_year)
    company_id = fields.Many2one( comodel_name="res.company", default=lambda self: self.env.user.company_id, string="Empresa")
    ...
    type = fields.Selection(
        [('balance', 'Notas ao Balanço'), ('demon', 'Notas a Demonstração ')], string='Tipo',default='balance')
    ...
    def print(self):
        if self.type == 'balance':
            return self.env.ref('es_account_report_ao.action_balance_notes_report').report_action(self)
        return self.env.ref('es_account_report_ao.action_demon_notes_report').report_action(self)
