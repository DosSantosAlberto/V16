...
class AccountPaymentAO(models.Model):
    ...
    def write(self, vals):
        ...
        if self.env.company.country_id.code == "AO":
            for pay in self:
                pay._check_data_payments()
                if pay.state == 'posted' and not pay.approved and pay.payment_type == 'outbound':
                    vals['payment_number'] = pay.env['ir.sequence'].with_company(pay.company_id.id).next_by_code(
                        'account.payment.sup', pay.date)
                    vals['approved'] = True
                elif pay.state == 'posted' and not pay.approved and pay.payment_type == 'inbound':
                    vals['payment_number'] = pay.env['ir.sequence'].with_company(pay.company_id.id).next_by_code(
                        'account.payment', pay.date)
                    vals['approved'] = True
                super(AccountPaymentAO, pay).write(vals)
    ...
    def _get_amount_in_words(self):
        for pay in self:
            ...
            if pay.amount > 0 and pay.currency_id:
                ...
                pay.amount_text = amount_to_text_pt.amount_to_text(pay.amount, currency_name)
    ...
    #             payment_status = 'A'
    #         line = payment.invoice_ids.mapped("invoice_line_ids")
    #
    #         payment = {
    #
    #             "PaymentRefNo": payment.payment_reference,
    #             "Period": int(fields.Date.to_string(payment.payment_date)[5:7]),
    #             "TransactionID": payment.id,
    #             "TransactionDate": fields.Date.to_string(payment.payment_date),
    #             "PaymentType": 'RC',
    #             "Description": payment.payment_reference,
    #             "SystemID": payment.name,
    #             "DocumentStatus": {
    #                 "PaymentStatus": payment_status,
    #                 "PaymentStatusDate": fields.Date.to_string(payment.payment_date),
    #                 "Reason": "",
    ...
    #                 "PaymentDate": fields.Date.to_string(payment.payment_date),
    #             },
    #             "SourceID": payment.create_uid.id,
    #             "SystemEntryDate": fields.Date.to_string(payment.write_date),
    #             "CustomerID": payment.partner_id.id,
    #             "Line": [
    #                 {
    #                     "LineNumber": inv.id,
    #                     "SourceDocumentID": {
    #                         "OriginatingON": inv.number,
    ...
    #                     "TaxExemptionReason": inv.tax_line_ids.filtered(
    #                         lambda r: r.amount == 0)[0].exemption_reason if inv.tax_line_ids.filtered(
    ...
    #
    #             } for inv in payment.invoice_ids for tax in
    ...
    #                 "open", "paid"] else 0
    #         result['Payments']['Payment'].append(payment)
    ...
    def _check_data_payments(self):
        if self.env['ir.config_parameter'].sudo().get_param('dont_validate_date'):
            return
        if self.move_type in ['out_invoice', 'out_refund']:
            if self.date > fields.Date.today():
                raise ValidationError(_("You can make a receipt for a date greater than today"))
        if self.date and self.move_type == "out_invoice" and self.env.user.company_id.country_id.code == "AO" and self.state == 'draft':
            payments = self.env['account.payment'].search([('state', '=', 'posted'),
                 ('journal_id', '=', self.journal_id.id), ('date', '!=', None), ('company_id', '=', self.company_id.id)],
                order="date desc", limit=1
            )
            last_payment_date = payments.filtered(lambda r: r.date > self.date)
            if last_payment_date:
                raise ValidationError(_("Can not create invoices, whose date is before date %s") % (
                    fields.Date.to_string(last_payment_date[0].date)))
