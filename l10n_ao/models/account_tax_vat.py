...
class AccountVATTax(models.Model):
    ...
    def compute_settlement(self):
        ...
        for tax_id in tax_groups:
            ...
            if deductible_balance > 0:
                ...
                for tax_account_id in tax_id.invoice_repartition_line_ids.filtered(lambda c: c.account_id.code):
                    ...
                    dedu_ml = self._get_double_move_line_vals(debit_account_id, credit_account_id, abs(deductible_balance),
                                                              move_id)
        ...
        for tax_id in tax_groups:
            regularization_balance = sum(
                [ml.debit - ml.credit for ml in
                 regularization_lines.filtered(lambda r: r.account_id.id == tax_id.refund_repartition_line_ids.mapped("account_id").id)])
        ...
        for tax_id in tax_groups:
            ...
            if paid_balance < 0:
                for tax_account_id in tax_id.invoice_repartition_line_ids.filtered(lambda c: c.account_id.code == '345311'):
